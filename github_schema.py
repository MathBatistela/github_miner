...
class Migration(sgqlc.types.Interface):
    __schema__ = github_schema
    __field_names__ = ('continue_on_error', 'created_at', 'failure_reason', 'id', 'migration_log_url', 'migration_source', 'repository_name', 'source_url', 'state')
...
class Issue(sgqlc.types.Type, Node, Assignable, Closable, Comment, Updatable, UpdatableComment, Labelable, Lockable, Reactable, RepositoryNode, Subscribable, UniformResourceLocatable, ProjectNextOwner, ProjectV2Owner):
    __schema__ = github_schema
    __field_names__ = ('body_resource_path', 'body_url', 'comments', 'hovercard', 'is_pinned', 'is_read_by_viewer', 'milestone', 'number', 'participants', 'project_cards', 'project_next_items', 'state', 'timeline_items', 'title', 'title_html', 'tracked_in_issues', 'tracked_issues', 'tracked_issues_count')
...
class ProjectView(sgqlc.types.Type, Node):
    __schema__ = github_schema
    __field_names__ = ('created_at', 'database_id', 'filter', 'group_by', 'items', 'layout', 'name', 'number', 'project', 'sort_by', 'updated_at', 'vertical_group_by', 'visible_fields')
    created_at = sgqlc.types.Field(sgqlc.types.non_null(DateTime), graphql_name='createdAt')
    database_id = sgqlc.types.Field(Int, graphql_name='databaseId')
